...
class CustomCutout(object):
    def __init__(self, max_h_size=0.2, max_w_size=0.2, prob=0.5):
    ...
    def __call__(self, results):
        ...
        if np.random.uniform(0, 1) < self.prob:
            ...
            num_boxes = np.random.randint(1, len(bboxes) + 1)
            
            img_cutout = cutout(img, bboxes, self.max_h_size, self.max_w_size, num_boxes)
            results['img'] = img_cutout

        return results

def cutout(image, boxes, max_h_size=0.2, max_w_size=0.2, num_boxes = 1):
    h, w, _ = image.shape
    
    if len(boxes) == 0 or num_boxes == 0:
        return image
    
    image_cutout = image.copy()

    selected_boxes = np.random.choice(len(boxes), size = num_boxes, replace = False)
    
    for box_idx in selected_boxes:
        box = boxes[box_idx]
        x1, y1, x2, y2 = box.astype(np.int32)
        cutout_max_h_size = min(max_h_size * h, y2 - y1)
        cutout_max_w_size = min(max_w_size * w, x2 - x1)
        cutout_h_size = np.random.uniform(0, cutout_max_h_size)
        cutout_w_size = np.random.uniform(0, cutout_max_w_size)
        cutout_center_x = np.random.randint(x1, x2+1)
        cutout_center_y = np.random.randint(y1, y2+1)
        cutout_half_h_size = int(cutout_h_size / 2)
        cutout_half_w_size = int(cutout_w_size / 2)
        image_cutout[cutout_center_y - cutout_half_h_size:cutout_center_y + cutout_half_h_size,
                     cutout_center_x - cutout_half_w_size:cutout_center_x + cutout_half_w_size, :] = 0

    return image_cutout

@PIPELINES.register_module()
class CustomCutout_V2(object):
    ...
    def __call__(self, results):
        ...
        if np.random.uniform(0, 1) < self.prob:
            ...
            img_cutout = cutout_v2(img, bboxes, num_boxes)
            results['img'] = img_cutout

        return results

def cutout_v2(image, boxes, num_boxes = 1):
    h, w, _ = image.shape
    ...
    for box_idx in selected_boxes:
        ...
        if p <= 0.33:
            # 중앙을 기준으로 위쪽만 가리도록
            cutout_center_x = (x1  + x2) // 2
            cutout_center_y = (y1 + y2) // 2
            image_cutout[y1:cutout_center_y, x1:x2+1, :] = 0
                
        elif p > 0.33 and p <=0.66:
            # 좌측만 가리도록
            cutout_left_x = (x1 + x2) // 4
            image_cutout[y1:y2+1, x1:cutout_left_x, :] = 0
        
        else :
            # 우측만 가리도록
            cutout_right_x = 3 * (x1 + x2) // 4
            image_cutout[y1:y2+1, cutout_right_x:x2+1, :] = 0
            
    return image_cutout

@PIPELINES.register_module()
class CustomCutout_V3(object):
    def __init__(self, prob=0.5):
        self.prob = prob
    ...
    def __call__(self, results):
        ...
        if np.random.uniform(0, 1) < self.prob:
            ...
            img_cutout = cutout_v3(img, bboxes, num_boxes)
            results['img'] = img_cutout

        return results

def cutout_v3(image, boxes, num_boxes = 1):
    h, w, _ = image.shape
    ...
    for box_idx in selected_boxes:
        ...
        p = np.random.randint(0,8 + 1)
      
        if p == 0 or p == 2:
            # 중앙을 기준으로 위쪽만 가리도록
            cutout_center_x = int((x1  + x2)*0.5)
            cutout_center_y = int((y1 + y2)*0.5)
            image_cutout[y1:cutout_center_y, x1:x2+1, :] = 0
            ...
            cutout_left_x = int(cutout_center_x - delta_x)
            image_cutout[y1:y2+1, x1:cutout_left_x, :] = 0
            ...
            cutout_right_x = int(cutout_center_x + delta_x)
            image_cutout[y1:y2+1, cutout_right_x:x2+1, :] = 0

        elif p == 4 or p == 7:
            # 차의 얼굴 부분만 가리도록
            cutout_center_y = (y1 + y2) // 2
            delta_y = int((y2 - y1)*0.3) 

            cutout_bottom_y = int(cutout_center_y + delta_y)
            image_cutout[cutout_center_y:cutout_bottom_y+1, x1:x2+1, :] = 0
            
    
    return image_cutout

@PIPELINES.register_module()
class CustomCutout_V4(object):
    def __init__(self, prob=0.5):
        self.prob = prob
    ...
    def __call__(self, results):
        ...
        if np.random.uniform(0, 1) < self.prob:
            ...
            img_cutout = cutout_v4(img, bboxes, num_boxes)
            results['img'] = img_cutout

        return results

def cutout_v4(image, boxes, num_boxes = 1):
    h, w, _ = image.shape
    ...
    for box_idx in selected_boxes:
        ...
        if p == 0 or p == 2:
            ...
            # 차의 로고만 가리도록
            # train 상에서 차가 사선일 때를 고려해서 threshold 찾기
            cutout_center_y = int((y1 + y2)*0.5)
            cutout_center_x = int((x1 + x2)*0.5)
            delta_y = int((y2 - y1)*0.4) 
            delta_x = int((x2 - x1)*0.1)

            cutout_bottom_y = int(cutout_center_y + delta_y)
            cutout_x1 = int(cutout_center_x - delta_x)
            cutout_x2 = int(cutout_center_x + 3 * delta_x)
            image_cutout[cutout_center_y:cutout_bottom_y+1, cutout_x1:cutout_x2+1, :] = 0
            
    return image_cutout

@PIPELINES.register_module()
class CustomCutout_V5(object):
    def __init__(self, prob=0.5):
        self.prob = prob
    ...
    def __call__(self, results):
        ...
        if np.random.uniform(0, 1) < self.prob:
            ...
            img_cutout = cutout_v5(img, bboxes, num_boxes)
            results['img'] = img_cutout

        return results

def cutout_v5(image, boxes, num_boxes = 1):
    h, w, _ = image.shape
